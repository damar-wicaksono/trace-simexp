...
Conducting a simulation experiment on TRACE code using ``trace-simexp`` is divided into three main steps:
...
1. The base TRACE input deck, in which the select parameters are to be perturbed.
2. The list of parameters file, where the perturbation factors are to be defined.
3. The design matrix file, which contains the value of perturbation factor normalized, between [0,1].

In this tutorial it is assumed that the base TRACE input deck for the FEBA facility model is already prepared (given in the tutorial package as ``febaTrans216.inp``).
Furthermore, the values of perturbation factor is generated by a certain experimental design.
``trace-simexp`` accepts normalized experimental design matrix with different samples written in rows and different parameters written in columns.
Below is an example of the first 5 entries in 7-dimensional latin hypercube experimental design::
...
The generation of such a design can be done by generic experimental design package 
and it is outside the scope of this package.
...
It requires basic knowledge on TRACE input deck.
The file ``feba216Vars7.inp`` has the following contents::
...
The line starting with ``#`` is comment line which will not be parsed.
The rest of the columns is as follows (note that the ordering has to be strictly kept):
...
2. the second column is the ``trace-simexp`` data type of the parameter.
   The data type is associated with TRACE components.
...
7. the seventh column is the ``word`` number of the variable.
   ``word`` is another TRACE input deck terminology used to designate the column at the which the variable is to be specified.
...
9. the ninth column is the probability distribution type of the perturbation factor.
   It is used to rescale the normalized value given in the design matrix file to the actual value of model parameter perturbation.
...
10. the tenth column is the parameters of the distribution given as a set of keyword-value pairs enclosed in square brackets.
    ...
    a uniform distribution 2 parameters need to be specified (the minimum and the maximum),
    while discrete distribution requires the choices and their respective probabilities to be specified.
...
The results of invoking above command is a set of perturbed TRACE input decks inside a directory structure::
...
To post-process the resulting ``dmx`` file, 
the user need to specify a  *list of TRACE graphic variable file* 
...
It is a good idea to include the comments using ``#`` either in-line or otherwise.
...
In this example, the process is not send in the background because having short list of extracted variables as above is not CPU demanding and can be waited.
...
Below gives some examples of how the results can be processed further.
...
In the figure below, the clad temperature evolution at 8 different axial locations 

.. image:: ../../figures/feba_grid.png

As it can be seen the dispersion given the model parameters variations are relatively minor.
The dispersion also tends to widen going from the bottom to the top indicating.
...
which in the context of reflood simulation are considered minor.
There are no apparent output d from the scatter plots except for the two blockage ratio, 
with the mixing vane blockage ratio showed a very strong correlation with the maximum temperature.

.. image:: ../../figures/temp_max.png

A similar set of scatter plots now for the time of quenching at elevation :math:`3.86` [m] as the quantity of interest is given below.
The range of variation in the sample is approximately between :math:`450` [s] and :math:`490` [s].
Similar to the results for maximum temperature, only the two blockage ratios showed strong correlation with the time of quenching.
